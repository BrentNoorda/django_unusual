...
<%
    ...
    if 'favorite' in request.GET:
        ...
        current_temperature = do_over( lambda x: lib.oakland_weather.get_current_oakland_weather(fail_sometimes_on_purpose=True), logger=logger )
            ...
            % for name,rgb in gColorRGBs.items():
                <tr>
                    <td style="background-color:${ rgbattr(rgb) }">
                        <a href="./favorite_color_with_temperature_and_db_do_over.mako?favorite=${ name | u}" style="color:${rgbattr(rgb ^ 0xFFFFFF)}">
                            ${ name | h}
                        </a>
                    </td>
                    <td>
