...
<html lang="en">
    ...
    <body>
        ...
        % if favorite_color is not None:
            <p>You selected ${ favorite_color | h } and the temperature is being retrieved in a background process.</p>
            ...
            % for name,v in gColors.items():
                <tr>
                    <td style="background-color:${ rgbattr(v['rgb'] )}">
                        <a href="./favorite_color_with_temperature_spawned.mako?favorite=${ name | u}" style="color:${rgbattr(v['rgb'] ^ 0xFFFFFF)}">
                            ${ name | h}
                        </a>
                    </td>
                    <td>
