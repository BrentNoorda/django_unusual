...
Where "-k gevent" tells gunicorn to use gevent to handle each request. "-w 2" start two workers (you want to have at least one for each CPU core so none are idle--for this demo one will suffice).
